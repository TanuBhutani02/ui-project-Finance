...
export default function Dashboard() {
  ...
  const {employeeList, employeeColumns} = useEmployee();
  const [activeTab, setActiveTab] = useState("dashboard");
  const dashboardData = selectedProjects?.length === 1 ? billingData[0]: billingData;
  useEffect(() => {
    ...
    <main className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-6">
      <div className="w-full flex space-x-4 border-b pb-2">
        {[
          ...
          { tab: "portfolio", label: "📁 Portfolio Details" },
          { tab: "employee", label: "👨‍💻 Employee Details 👩‍💻" }
        ].map(({ tab, label }) => (
          <button
            key={tab}
          ...
          {dashboardData ? (
            <div className="grid grid-cols-1 gap-6 mt-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  { label: "💰 Total Revenue", value: dashboardData?.total_revenue?.toFixed(2) || "0.00", color: "blue" },
                  { label: "💸 Total Cost", value: dashboardData?.total_cost_to_account?.toFixed(2) || "0.00", color: "indigo" },
                  { label: "📈 Gross Margin", value: dashboardData?.gross_Margin?.toFixed(2) || "0.00", color: "green" },
                  { label: "🧑‍💻 Utilization Billable Only", value: dashboardData?.billing_Utilization?.toFixed(2) + " %" || "0.00", color: "blue" },
                  { label: "⏳ Overall Utilization", value: dashboardData?.overall_Utilization?.toFixed(2) + " %" || "0.00", color: "indigo" },
                  { label: "🪙 Gross Margin Percentage", value: dashboardData?.gross_Margin_Percent?.toFixed(2) + " %" || "0.00", color: "green" },
              ...
              <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
                <h2 className="text-xl font-semibold text-center mb-4">📊 Project Distribution</h2>
                <ProjectPieChart project={dashboardData} />
              </div>
      ...
      {activeTab === "employee" && (
        <div className="mt-6 text-center">
          ...
          <DataGridComponent rows={employeeList} columns={employeeColumns} />
        </div>
      )}
    </main>
  );
